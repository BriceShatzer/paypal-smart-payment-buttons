...
function getFundingEligibilityParam(req) {
  ...
  if (encodedFundingEligibility && typeof encodedFundingEligibility === 'string') {
    ...
    try {
      ...
      throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Invalid funding eligibility: ${encodedFundingEligibility}`, err);
    }
...
function getPaymentMethodNonce(req) {
...
function getLocale(params) {
  ...
  if (!langs) {
    throw (0, _lib.makeError)(_config.ERROR_CODE.VALIDATION_ERROR, `Invalid locale country: ${country}`);
  }

  lang = lang || langs[0];

  if (langs.indexOf(lang) === -1) {
    throw (0, _lib.makeError)(_config.ERROR_CODE.VALIDATION_ERROR, `Invalid locale language: ${lang}`);
  }
...
function getButtonPreflightParams(params) {
  ...
  if (!clientID) {
    throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Please provide a ${_sdkConstants.SDK_QUERY_KEYS.CLIENT_ID} query parameter`);
  }

  if (!userIDToken) {
    throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Please provide a ${_constants.SPB_QUERY_KEYS.USER_ID_TOKEN} query parameter`);
  }

  for (const merchant of merchantID) {
    if (!merchant.match(/^[A-Z0-9]+$/)) {
      throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Invalid ${_sdkConstants.SDK_QUERY_KEYS.MERCHANT_ID} query parameter`);
    }
  }

  if (currency && !(0, _belter.constHas)(_sdkConstants.CURRENCY, currency)) {
    throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Invalid ${_sdkConstants.SDK_QUERY_KEYS.CURRENCY} query parameter`);
  }

  if (amount && !amount.match(/^\d+\.\d{2}$/)) {
    throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Invalid ${_constants.SPB_QUERY_KEYS.AMOUNT} query parameter`);
  }
