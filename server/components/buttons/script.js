...
async function getPayPalSmartPaymentButtonsRenderScript({
  ...
  const button = await watcher.importDependency(_config.CHECKOUT_COMPONENTS_MODULE, _config.BUTTON_RENDER_JS, _config.ACTIVE_TAG);
  return {
    button,
    version
  };
...
async function getSmartPaymentButtonsClientScript({
  ...
  const script = await watcher.read(debug ? _config.BUTTON_CLIENT_JS : _config.BUTTON_CLIENT_MIN_JS, _config.ACTIVE_TAG);
  return {
    script,
    version
  };
