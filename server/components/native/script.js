...
async function getNativePopupClientScript({
  ...
  const script = await watcher.read(debug ? _config.NATIVE_POPUP_CLIENT_JS : _config.NATIVE_POPUP_CLIENT_MIN_JS, _config.ACTIVE_TAG);
  return {
    script,
    version
  };
...
async function getNativePopupRenderScript({
  ...
  const popup = await watcher.import(debug ? _config.NATIVE_POPUP_CLIENT_JS : _config.NATIVE_POPUP_CLIENT_MIN_JS, _config.ACTIVE_TAG);
  return {
    popup,
    version
  };
...
async function getNativeFallbackClientScript({
  ...
  const script = await watcher.read(debug ? _config.NATIVE_FALLBACK_CLIENT_JS : _config.NATIVE_FALLBACK_CLIENT_MIN_JS, _config.ACTIVE_TAG);
  return {
    script,
    version
  };
...
async function getNativeFallbackRenderScript({
  ...
  const fallback = await watcher.import(debug ? _config.NATIVE_FALLBACK_CLIENT_JS : _config.NATIVE_FALLBACK_CLIENT_MIN_JS, _config.ACTIVE_TAG);
  return {
    fallback,
    version
  };
