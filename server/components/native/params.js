...
function getParentDomain(params) {
  ...
  if (!parentDomain) {
    throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Parent domain not passed`);
  }

  if (typeof parentDomain !== 'string') {
    throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Expected parentDomain param to be a string`);
  } // eslint-disable-next-line security/detect-unsafe-regex


  if (process.env.NODE_ENV !== 'development' && !parentDomain.match(/\.paypal\.com(:\d{1,4})?$/)) {
    throw new _lib.makeError(_config.ERROR_CODE.VALIDATION_ERROR, `Expected paypal parentDomain`);
  }
